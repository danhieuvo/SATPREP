-- SAT Quest squad server. Paste this whole file into Supabase > SQL Editor > New query, then Run.
-- Everyone can READ squad rows (names, XP, streaks, feed). All WRITES go through the functions
-- below, which check each player's device secret, so nobody can change another player's XP.
...
-- Full progress backup (used by transfer codes). Never readable directly.
...
create policy "read squads" on sq_squads for select using (true);
create policy "read players" on sq_players for select using (true);
create policy "read events" on sq_events for select using (true);

revoke all on sq_squads, sq_players, sq_private, sq_events from anon, authenticated;
grant select on sq_squads, sq_players, sq_events to anon, authenticated;
...
begin
  ...
  values (p_id, (select code from sq_squads where code = p_squad), p_public, now())
...
begin
  ...
  if not exists (select 1 from sq_players where id = p_id and squad = p_squad) then raise exception 'not in squad'; end if;
...
begin
  ...
  where id = p_event and not (p_id = any(kudos))
    and squad = (select squad from sq_players where id = p_id);
end $$;
